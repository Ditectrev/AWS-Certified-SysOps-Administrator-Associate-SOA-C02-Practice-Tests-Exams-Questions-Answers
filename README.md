...
- 🛒 [eBay, in PDF format, with answers marked](https://www.ebay.com/usr/ditectrev),
- 🛒 [eBay, in PDF format, without answers marked](https://www.ebay.com/usr/ditectrev),
...
### A SysOps Administrator is managing a `VPC` network consisting of public and private subnets. Instances in the private subnets access the Internet through a `NAT` gateway. A recent AWS bill shows that the `NAT` gateway charges have doubled. The Administrator wants to identify which instances are creating the most network traffic. How should this be accomplished?
